...
import cvut.fit.kot.data.useCase.UpdateClientProfileUseCase
...
class EditProfileViewModel @Inject constructor(
    ...
    fun load() = viewModelScope.launch {
        ...
        runCatching {
            val profile   = async { getProfile.execute().getOrThrow() }
            val locations = async { getLocations.execute().getOrThrow() }
    ...
    fun save(onOk: () -> Unit) = viewModelScope.launch {
        ...
        runCatching { updateProfile.execute(req) }
            .onSuccess { updated ->
